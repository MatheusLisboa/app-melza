-- =============================================================================
-- Continuidade: 002 parou ao dropar is_family_member (policies antigas dependiam dela)
-- Estado esperado: tabelas já renomeadas (workspaces, workspace_id, …)
-- Rode SÓ este arquivo no SQL Editor (não precisa re-rodar 002).
-- =============================================================================

-- 1) Remove policies antigas (nomes family_* ainda no schema renomeado)
DROP POLICY IF EXISTS families_select ON workspaces;
DROP POLICY IF EXISTS families_insert ON workspaces;
DROP POLICY IF EXISTS families_update ON workspaces;
DROP POLICY IF EXISTS workspaces_select ON workspaces;
DROP POLICY IF EXISTS workspaces_insert ON workspaces;
DROP POLICY IF EXISTS workspaces_update ON workspaces;

DROP POLICY IF EXISTS family_members_select ON workspace_members;
DROP POLICY IF EXISTS family_members_insert ON workspace_members;
DROP POLICY IF EXISTS family_members_update ON workspace_members;
DROP POLICY IF EXISTS workspace_members_select ON workspace_members;
DROP POLICY IF EXISTS workspace_members_insert ON workspace_members;
DROP POLICY IF EXISTS workspace_members_update ON workspace_members;

DROP POLICY IF EXISTS family_invites_select ON workspace_invites;
DROP POLICY IF EXISTS family_invites_select_member ON workspace_invites;
DROP POLICY IF EXISTS family_invites_select_token ON workspace_invites;
DROP POLICY IF EXISTS family_invites_insert ON workspace_invites;
DROP POLICY IF EXISTS family_invites_update ON workspace_invites;
...
-- 2) Agora pode dropar RPCs antigas
...
-- 5) Garante coluna type em workspaces (se rename veio de families sem type)
ALTER TABLE workspaces ADD COLUMN IF NOT EXISTS type TEXT;
UPDATE workspaces SET type = 'COUPLE' WHERE type IS NULL;
ALTER TABLE workspaces ALTER COLUMN type SET DEFAULT 'PERSONAL';
DO $$
BEGIN
  ALTER TABLE workspaces DROP CONSTRAINT IF EXISTS families_type_check;
...
EXCEPTION
  WHEN others THEN
    RAISE NOTICE 'workspaces type check: %', SQLERRM;
END $$;
